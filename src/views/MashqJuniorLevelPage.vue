...
// yordamchi: HTML belgilarini xavfsiz tarzda chiqarish uchun escape qilamiz
...
// yordamchi: $...$ va $$...$$ ichidagi matematikani HTML ga aylantiramiz
const renderMath = (source) => {
  ...
  const parts = []
  let cursor = 0
  // regex captures display ($$...$$) OR inline ($...$) math
  const pattern = /\$\$([\s\S]+?)\$\$|\$([^$\n]+?)\$/g
  let match = pattern.exec(source)

  while (match !== null) {
    // matematika bo'lagidan oldin kelgan oddiy matnni qo'shib qo'yamiz
    if (match.index > cursor) {
      parts.push(escapeHtml(source.slice(cursor, match.index)))
    }

    // ifoda — qaysi capture group mos kelgan bo'lsa, o'sha
    ...
    // $$...$$ shakli uchun display mode true bo'ladi
    let displayMode = false
    if (match[1] !== undefined) {
      displayMode = true
    }

    // matematikani render qilamiz, KaTeX xatolik bersa oddiy matnga qaytamiz
    try {
  ...
  // oxirgi matematika bo'lagidan keyingi oddiy matn
...
// har bir filterda open flag (dropdown ochiqmi) va tanlangan qiymat bor
...
// berilgan filter qiymati uchun label matnini topamiz
...
// har bir filter tugmasida ko'rinadigan label / badge matnlari
...
const gradeBadge = computed(() => {
  ...
  return resolveLabel(GRADE_OPTIONS, filters.grade.value)
})

const difficultyBadge = computed(() => {
  if (filters.difficulty.value === 'all') {
    return 'Hamma'
  }
  return resolveLabel(DIFFICULTY_OPTIONS, filters.difficulty.value)
})

const topicBadge = computed(() => {
  if (filters.topic.value === 'all') {
    return 'Hamma'
  }
  return resolveLabel(TOPIC_OPTIONS, filters.topic.value)
})
...
const statusBadge = computed(() => {
  ...
  return resolveLabel(STATUS_OPTIONS, filters.status.value)
})
...
// filter tugmasi bosildi: boshqalarini yopib, shuni almashtiramiz
const toggleFilter = (key) => {
  ...
  if (wasOpen) {
    ...
    filters[key].open = true
  }
}

// dropdown ichidagi elementga bosildi
const selectFilter = (key, value) => {
  filters[key].value = value
  filters[key].open = false
}

// hammasini boshlang'ich holatga qaytaramiz
const resetFilters = () => {
...
// dropdowndan tashqariga bosilsa, ochiqlarini yopamiz
...
// oldingi natijalar (pill stripda yashil savollar ko'rinishi uchun)
const questionStates = reactive({ 1: 'correct', 2: 'correct' })

// hozir ko'rsatilayotgan savol (hozircha mock ma'lumot)
...
// savol matni uchun oldindan tayyorlangan HTML
const questionHtml = computed(() => renderMath(question.text))

// har bir izoh paragrafi uchun oldindan tayyorlangan HTML
const explanationHtml = computed(() => {
  const out = []
  for (const paragraph of question.explanationParagraphs) {
    out.push(renderMath(paragraph))
  }
  return out
})

// HTML render qilingan variantlar ro'yxati
...
// ─── interaktiv holat ───────────────────────────────────────────────────
const selectedLetter = ref(null)      // foydalanuvchi qaysi harfni tanladi
const answered = ref(false)           // foydalanuvchi javob bermidimi
const wasCorrect = ref(false)         // tanlovi to'g'rimidi
const explanationOpen = ref(false)    // izoh paneli ochiqmi
const explanationRead = ref(false)    // foydalanuvchi izohni hech bo'lmaganda bir marta ochganmi
const saved = ref(false)              // saqlash tugmasi holati
const showNudge = ref(false)          // sariq "izohni o'qimadingiz" eslatmasi
const showSummary = ref(false)        // session yakunidagi modal
const correctCount = ref(9)           // xulosa ekrani uchun namunaviy raqamlar
const incorrectCount = ref(3)
const accuracyFillWidth = ref('0%')

// variant uchun qaysi CSS class kerakligini aniqlaymiz (selected / correct / incorrect / hech narsa)
const optionStateClass = (option) => {
  // foydalanuvchi tanlashidan oldin: faqat bosgan variantida "selected" ko'rsatamiz
  if (!answered.value) {
    if (selectedLetter.value === option.letter) {
      return 'selected'
    }
    return ''
  }

  // foydalanuvchi tanlagandan keyin: to'g'ri javobni yashil qilamiz
  if (option.correct) {
    return 'correct'
  }

  // ...va noto'g'risini (agar bosilgan bo'lsa) qizil qilamiz
...
// aniqlik = to'g'ri / (to'g'ri + noto'g'ri)
...
// progress = savollar bo'yicha qancha yo'l bosganimiz
const progressPercent = computed(() => {
  return Math.round((currentQuestionIndex.value / TOTAL_QUESTIONS) * 100)
})

// pill ro'yxatini quramiz (har bir savol uchun bitta pill)
const qstripPills = computed(() => {
  ...
  for (let i = 0; i < TOTAL_QUESTIONS; i++) {
    ...
    // bu pill uchun holatni aniqlaymiz
    let state = ''
    if (questionStates[number]) {
      state = questionStates[number]
    } else if (isCurrent) {
      state = 'current'
    }

    // CSS class ro'yxatini quramiz
    const classList = []
    if (state === 'correct') {
...
// sekundlarni mm:ss formatiga aylantiramiz
...
// javob berilgan savol uchun o'rtacha vaqt (xulosa modali uchun)
...
// ikkala interval taymerni ishga tushiramiz
const startTimers = () => {
  qInterval = setInterval(() => {
    // foydalanuvchi javob bergach savol taymerini to'xtatib turamiz
    if (!answered.value) {
      qSeconds.value += 1
    }
  }, 1000)
...
// ─── interaktiv handlerlar ──────────────────────────────────────────────
// variantga bosildi (yoki klaviatura yorlig'i ishlatildi)
...
// indeks 0..3 bo'yicha tanlash (raqam tugmalari yorlig'i ishlatadi)
...
// izoh panelini ko'rsatish/yashirish (faqat javob berilgandan keyin)
...
// saqlash tugmasini almashtirish
const toggleSaved = () => {
  if (saved.value) {
    saved.value = false
  } else {
...
// keyingi savol tugmasi — izoh ko'rilmasdan noto'g'ri javob o'tkazib yuborilishidan saqlaydi
const goNext = () => {
  // agar noto'g'ri javob berib, izohni ham ochmagan bo'lsalar, avval eslatma chiqaramiz
...
// eslatmadagi "o'tkazib yuborish" tugmasi — yopib, bu savolda boshqa eslatmalarni o'chiramiz
const skipNudge = () => {
  showNudge.value = false
  explanationRead.value = true
}

// eslatmadagi "o'qish" tugmasi — yopib, izohni ochamiz
...
// katta qizil "Mashqni tugatish" tugmasi — xulosa modalini ko'rsatamiz
const finishSession = () => {
  showSummary.value = true
  // modal ekranga chiqqach aniqlik chizig'ini animatsiya qilamiz
  nextTick(() => {
    accuracyFillWidth.value = `${accuracyPercent.value}%`
  })
}

// xulosa modalini yopamiz
const closeSummary = () => {
...
const onKeyDown = (event) => {
  // input/textarea ichida yozilayotgan matnga aralashmaymiz
  ...
  // 1..4 → indeks bo'yicha variant tanlash
  ...
  // a..d → harf bo'yicha variant tanlash
...
// hozirgi savol o'zgarganda, savolga oid barcha holatni qaytaramiz
