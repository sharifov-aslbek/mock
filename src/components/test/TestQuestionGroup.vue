    ...
    <div class="mt-5 space-y-5 sm:mt-6 sm:space-y-6">
      ...
      >
        ...
        >
          ...
          <button
            type="button"
            class="mt-1.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-[12px] border border-[#e1e5dc] bg-white shadow-[0_3px_10px_rgba(26,24,20,0.06)] transition hover:border-[#bcc7b5] hover:bg-[#fbfdf8] sm:mt-2"
            :class="isQuestionChecked(question.id) ? 'text-[#1a1814]' : 'text-[#7f8a78]'"
            aria-label="Save question"
            @click.stop="updateQuestionCheck(question.id, !isQuestionChecked(question.id))"
          >
            <svg class="h-6 w-6" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <path
                d="M7 4.75h10v14.5l-5-3.25-5 3.25V4.75Z"
                :fill="isQuestionChecked(question.id) ? 'currentColor' : 'none'"
                stroke="currentColor"
                stroke-width="2"
